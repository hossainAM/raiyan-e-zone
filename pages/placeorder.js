...
const placeOrder = () => {
    ...
    //check payment method is defined or not;
    ...
    //define handlePlaceOrder function
    const [loading, setLoading] = useState(false);

    const handlePlaceOrder = async () => {
        try{
            setLoading(true);
            //create POST API to add order data
            ...
            router.push(`/order/${data._id}`);//500 error- not directing to order page
        } catch(err) {
            setLoading(false);
